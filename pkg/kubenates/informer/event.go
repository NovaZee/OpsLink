...
func (e *EventInformer) OnAdd(obj interface{}, isInInitialList bool) {
	e.store(obj, false)
	logger.Infow("event informer webhook", "action", "OnAdd", "Message", obj.(*corev1.Event).Message)
}
func (e *EventInformer) OnUpdate(oldObj, newObj interface{}) {
	e.store(newObj, false)
	logger.Infow("event informer webhook", "action", "OnAdd", "Message", newObj.(*corev1.Event).Message)
}
